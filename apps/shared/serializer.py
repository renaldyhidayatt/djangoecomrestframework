...
class PageMetaSerializer(serializers.Serializer):
    ...
    def get_total_pages_count(self, instance):
        return math.ceil(
            self.get_total_items_count(instance)
            / self.get_requested_page_size(instance)
        )

    def get_has_next_page(self, resources):
        if self.get_current_page_number(resources) < self.get_total_pages_count(
            resources
        ):
            return True
        else:
    ...
    def get_next_page_url(self, resources):
        return "%s?page=%d&page_size=%d" % (
            ...
            self.get_next_page_number(resources),
    ...
    def get_prev_page_url(self, resources):
        return "%s?page=%d&page_size=%d" % (
            self.context.get("request").path,
            self.get_prev_page_number(resources),
            self.get_requested_page_size(resources),
        )
...
class PageMetaModel:
    def __init__(self, request, paginator):
        ...
        self.data["total_pages_count"] = math.ceil(
            self.data["total_items_count"] / self.data["requested_page_size"]
        )
